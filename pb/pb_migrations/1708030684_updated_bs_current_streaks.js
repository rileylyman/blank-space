...
migrate((db) => {
  ...
  // remove
  collection.schema.removeField("qbbg8gq8")

  // remove
  collection.schema.removeField("5ewctgl2")

  // remove
  collection.schema.removeField("7gqs1iiv")
  ...
  collection.options = {
    "query": "with played_days_ago as (\n  select distinct \n      bgp.user,\n      cast(julianday('now') - julianday(bgs.publish_on) as int) as days_ago\n    from bs_game_progress bgp\n    join bs_game_sets bgs \n        on json_extract(bgs.games, '$') like '%' || bgp.bs_game || '%'\n    where julianday(bgs.publish_on) <= julianday('now')\n),\nplayed_days_ago_with_rn as (\n  select \n      user,\n      days_ago,\n      (row_number() over(partition by user order by days_ago)) as rn\n    from played_days_ago\n),\nstreak_numbers as (\n  select \n      (row_number() over()) as id, \n      user,\n      days_ago,\n      rn,\n      (days_ago - rn + 1) as streak_number\n    from played_days_ago_with_rn\n)\nselect\n  id,\n  user,\n  streak_number,\n  (count(streak_number) over(partition by user,streak_number)) as cnt\nfrom streak_numbers"
  }
  ...
  // remove
  collection.schema.removeField("22vyqbte")

  // remove
  collection.schema.removeField("umxm6f9t")

  // remove
  collection.schema.removeField("lbybh9sj")
