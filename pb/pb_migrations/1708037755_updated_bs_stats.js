...
migrate((db) => {
  ...
  // remove
  collection.schema.removeField("jayo9iek")

  // remove
  collection.schema.removeField("trtpvgtb")

  // remove
  collection.schema.removeField("s16zv5fr")

  // remove
  collection.schema.removeField("9bevgpvj")

  // remove
  collection.schema.removeField("f7guacnu")

  // remove
  collection.schema.removeField("haqb0iat")
  ...
  collection.options = {
    "query": "with played_days_ago as (\n  select distinct \n      bgp.user,\n      cast(julianday('now') - julianday(bgs.publish_on) as int) as days_ago\n    from bs_game_progress bgp\n    join bs_game_sets bgs \n        on json_extract(bgs.games, '$') like '%' || bgp.bs_game || '%'\n    where julianday(bgs.publish_on) <= julianday('now')\n),\nplayed_days_ago_with_rn as (\n  select \n      user,\n      days_ago,\n      (row_number() over(partition by user order by days_ago)) as rn\n    from played_days_ago\n),\nstreak_numbers as (\n  select \n      (row_number() over()) as id, \n      user,\n      days_ago,\n      rn,\n      (days_ago - rn + 1) as streak_number\n    from played_days_ago_with_rn\n),\nstreak_lengths as (\n  select distinct\n    user,\n    streak_number,\n    (count(streak_number) over(partition by user, streak_number)) as streak_length\n  from streak_numbers\n),\nmax_streaks as (\n  select\n    user,\n    max(streak_length) as max_streak\n  from streak_lengths\n  group by user\n),\ncurrent_streak as (\n  select\n    user,\n    min(streak_number) as streak_number,\n    streak_length as current_streak\n  from streak_lengths\n  where streak_number = 1 or streak_number = 0\n  group by user\n),\ntotal_games as (\n  select\n    user,\n    count(*) as total_games\n  from bs_game_progress\n  where won = 1 or lost = 1\n  group by user\n),\nwon_games as (\n  select\n    user,\n    count(*) as won_games\n  from bs_game_progress\n  where won = 1\n  group by user\n),\nnum_guesses as (\n  select\n    user,\n    length(guesses) - length(replace(guesses, ',', '')) as num_guesses,\n    count(*) as guess_distro\n  from bs_game_progress\n  group by user, num_guesses\n)\nselect\n  (row_number() over()) as id,\n  users.id as user,\n  maxs.max_streak as max_streak,\n  currs.current_streak as current_streak,\n  tot.total_games,\n  won.won_games,\n  ng.guess_distro\nfrom users\nleft join num_guesses ng on ng.user = users.id\nleft join total_games tot on tot.user = users.id\nleft join current_streak currs on currs.user = users.id\nleft join max_streaks maxs on maxs.user = users.id\nleft join won_games won on won.user = users.id"
  }
  ...
  // remove
  collection.schema.removeField("5mjhmzzl")

  // remove
  collection.schema.removeField("pmrarzca")

  // remove
  collection.schema.removeField("rbc6jp0z")

  // remove
  collection.schema.removeField("cr8typib")

  // remove
  collection.schema.removeField("xiulresx")

  // remove
  collection.schema.removeField("53koymcs")

  // remove
  collection.schema.removeField("pezz4opg")

  // remove
  collection.schema.removeField("3b6melax")

  // remove
  collection.schema.removeField("klydvx5p")

  // remove
  collection.schema.removeField("vbajodum")
